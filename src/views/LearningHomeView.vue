...
// ── 學習入口設定（未來可移至資料庫） ──────────────────────────
interface PortalSubCategory {
  id: string
  label: string
  icon: string
  description: string
  categoryNames: string[] | null   // null = 顯示全部
}
...
interface PortalCategory {
  ...
  color: string
  textColor: string
  subCategories: PortalSubCategory[] | null   // null = 直接顯示課程
...
const portalConfig: PortalRole[] = [
    ...
    categories: [
      {
        ...
        label: '新人培訓',
        icon: '🌱',
        description: '剛加入富康大家庭？從這裡開始！',
        color: '#10b981',
        textColor: '#fff',
        subCategories: null,
        categoryNames: null,   // 先顯示全部課程，之後可設定 ['新人訓練']
      },
      {
        id: 'supervisor',
        label: '主管課程',
        icon: '👔',
        description: '提升管理能力與工具應用',
        color: '#6366f1',
        textColor: '#fff',
        subCategories: [
          {
          ...
          },
          {
            ...
            label: '工具使用',
            icon: '🔧',
            description: 'Google Workspace、系統操作、數位工具',
            categoryNames: null,
          },
        ],
        categoryNames: null,
  ...
  },
  {
    ...
    bgClass: 'role-hq',
    enabled: false,
    comingSoonText: '總部課程規劃中，敬請期待',
    categories: [],
  },
]

// ── 導覽狀態 ──────────────────────────────────────────────────
type Level = 'role' | 'category' | 'subcategory' | 'courses'
const currentLevel = ref<Level>('role')
const selectedRole = ref<PortalRole | null>(null)
const selectedCategory = ref<PortalCategory | null>(null)
const selectedSubCategory = ref<PortalSubCategory | null>(null)
const isAnimating = ref(false)

function navigateTo(level: Level, role?: PortalRole, cat?: PortalCategory, sub?: PortalSubCategory) {
  if (isAnimating.value) return
  isAnimating.value = true
  if (role !== undefined) selectedRole.value = role
  if (cat !== undefined) selectedCategory.value = cat
  if (sub !== undefined) selectedSubCategory.value = sub
  currentLevel.value = level
  setTimeout(() => { isAnimating.value = false }, 350)
}
...
function selectRole(role: PortalRole) {
  ...
  navigateTo('category', role)
}

function selectCategory(cat: PortalCategory) {
  if (cat.subCategories && cat.subCategories.length > 0) {
    navigateTo('subcategory', selectedRole.value!, cat)
  } else {
    navigateTo('courses', selectedRole.value!, cat)
  }
}

function selectSubCategory(sub: PortalSubCategory) {
  navigateTo('courses', selectedRole.value!, selectedCategory.value!, sub)
}

function goBack() {
  if (currentLevel.value === 'courses') {
    if (selectedCategory.value?.subCategories?.length) {
...
const activeCategoryNames = computed<string[] | null>(() => {
  if (currentLevel.value !== 'courses') return null
  const sub = selectedSubCategory.value
  const cat = selectedCategory.value
  if (sub?.categoryNames) return sub.categoryNames
  if (cat?.categoryNames) return cat.categoryNames
  return null   // null = 全部
})

const filteredCourses = computed(() => {
  const names = activeCategoryNames.value
  if (!names) return visibleCourses.value
  const matchIds = new Set(
    catalog.categories.filter((c) => names.includes(c.name)).map((c) => c.id),
  )
  return visibleCourses.value.filter((c) => matchIds.has(c.categoryId ?? -1))
})
...
function getProgress(courseId: string) {
  ...
  return Math.round(percentages.reduce((a, b) => a + b, 0) / course.lessons.length)
}

// ── 麵包屑 ────────────────────────────────────────────────────
const breadcrumbs = computed(() => {
  const crumbs: Array<{ label: string; level: Level }> = [{ label: '學習入口', level: 'role' }]
  if (selectedRole.value && currentLevel.value !== 'role') {
    crumbs.push({ label: selectedRole.value.label, level: 'category' })
  }
  if (selectedCategory.value && (currentLevel.value === 'subcategory' || currentLevel.value === 'courses')) {
    crumbs.push({ label: selectedCategory.value.label, level: 'subcategory' })
  }
  if (selectedSubCategory.value && currentLevel.value === 'courses') {
    crumbs.push({ label: selectedSubCategory.value.label, level: 'courses' })
  }
  return crumbs
})
</script>
...
<template>
  <div class="portal-wrap">
    ...
    <nav v-if="currentLevel !== 'role'" class="portal-breadcrumb">
      ...
      <span v-for="(crumb, i) in breadcrumbs" :key="crumb.level" class="crumb">
        ...
        <span
          class="crumb-label"
          :class="{ 'crumb-active': i === breadcrumbs.length - 1 }"
          @click="i < breadcrumbs.length - 1 ? navigateTo(crumb.level) : null"
        >{{ crumb.label }}</span>
      </span>
    </nav>
      ...
      <div v-if="currentLevel === 'role'" class="portal-level">
        ...
        <div class="role-grid">
          <button
            ...
            :key="role.id"
            class="role-card"
            :class="[role.bgClass, { 'role-card--disabled': !role.enabled }]"
            @click="selectRole(role)"
          >
            <div class="role-card__icon">{{ role.icon }}</div>
            <div class="role-card__body">
              <h3>{{ role.label }}</h3>
              <p>{{ role.comingSoonText ?? role.description }}</p>
            </div>
            <div v-if="!role.enabled" class="coming-soon-badge">即將推出</div>
            <div v-else class="role-card__arrow">→</div>
          </button>
        </div>
      </div>
    </Transition>
    ...
    <!-- ─ Level 1：分類選擇 ─ -->
    <Transition name="slide-up">
      <div v-if="currentLevel === 'category' && selectedRole" class="portal-level">
        <div class="portal-header">
          <span class="portal-role-badge" :style="{ background: selectedRole.accentColor }">
            {{ selectedRole.icon }} {{ selectedRole.label }}
          </span>
          <h2 class="portal-title">選擇學習方向</h2>
          <p class="portal-subtitle">點擊下方分類，進入對應的課程</p>
        </div>
        <div class="category-grid">
          <button
            v-for="(cat, i) in selectedRole.categories"
            :key="cat.id"
            class="category-card"
            :style="{ '--card-color': cat.color, animationDelay: `${i * 80}ms` }"
            @click="selectCategory(cat)"
          >
            <div class="category-card__icon">{{ cat.icon }}</div>
            <h3 class="category-card__label">{{ cat.label }}</h3>
            <p class="category-card__desc">{{ cat.description }}</p>
            <div class="category-card__arrow">
              {{ cat.subCategories ? '選擇分類 →' : '進入課程 →' }}
            </div>
          </button>
        </div>
      </div>
    ...
    <!-- ─ Level 2：子分類選擇 ─ -->
    <Transition name="slide-up">
      <div v-if="currentLevel === 'subcategory' && selectedCategory?.subCategories" class="portal-level">
        <div class="portal-header">
          <span class="portal-role-badge" :style="{ background: selectedCategory.color }">
            {{ selectedCategory.icon }} {{ selectedCategory.label }}
          </span>
          <h2 class="portal-title">選擇課程類別</h2>
        </div>
        <div class="subcategory-grid">
          <button
            v-for="(sub, i) in selectedCategory.subCategories"
            :key="sub.id"
            class="subcategory-card"
            :style="{ animationDelay: `${i * 80}ms` }"
            @click="selectSubCategory(sub)"
          >
            <div class="subcategory-card__icon">{{ sub.icon }}</div>
            <h3>{{ sub.label }}</h3>
            <p>{{ sub.description }}</p>
            <span class="subcategory-card__cta">進入課程 →</span>
          </button>
        </div>
      </div>
    </Transition>
    ...
    <Transition name="slide-up">
      <div v-if="currentLevel === 'courses'" class="portal-level">
        <div class="portal-header">
          <h2 class="portal-title">
            {{ selectedSubCategory?.label ?? selectedCategory?.label ?? '全部課程' }}
          </h2>
          <p class="portal-subtitle">{{ filteredCourses.length }} 個課程</p>
        </div>
        <p v-if="filteredCourses.length === 0" class="empty-courses">
          此分類目前尚無課程，請靜待內容更新。
        </p>
        <section class="course-grid">
          <RouterLink
...
.portal-role-badge {
  display:inline-flex; align-items:center; gap:.35rem;
  padding:.25rem .75rem; border-radius:2rem; font-size:.82rem;
...
.role-card:hover:not(.role-card--disabled) { transform:translateY(-4px); box-shadow:0 12px 30px rgba(0,0,0,.35); }
...
.coming-soon-badge {
  ...
  border-radius:2rem; padding:.2rem .65rem; font-size:.72rem; font-weight:600;
}
...
.category-card:hover { transform:translateY(-3px) scale(1.01); box-shadow:0 10px 28px rgba(0,0,0,.35); }
.category-card__icon { font-size:2rem; }
.category-card__label { font-size:1.05rem; font-weight:700; margin:0; }
.category-card__desc { font-size:.82rem; opacity:.85; margin:0; flex:1; }
.category-card__arrow { font-size:.8rem; opacity:.75; font-weight:600; margin-top:.25rem; }

/* ── 子分類卡 ── */
.subcategory-grid { display:grid; grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)); gap:1rem; }
.subcategory-card {
  display:flex; flex-direction:column; align-items:flex-start; gap:.4rem;
  padding:1.4rem 1.25rem; border-radius:1rem; cursor:pointer; text-align:left;
  background:rgba(255,255,255,.07); border:1px solid rgba(255,255,255,.12);
  transition:transform .2s, background .2s;
  animation:pop-in .35s ease both;
}
.subcategory-card:hover { transform:translateY(-2px); background:rgba(255,255,255,.12); }
.subcategory-card__icon { font-size:1.75rem; }
.subcategory-card h3 { margin:0; font-size:1rem; font-weight:700; color:#f9fafb; }
.subcategory-card p { margin:0; font-size:.82rem; color:#9ca3af; flex:1; }
.subcategory-card__cta { font-size:.78rem; color:#f59e0b; font-weight:600; }
